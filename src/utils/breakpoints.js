...
function up(theme: Theme, device: Device): string {
  return `@media screen ${getMinWidthQuery(theme, device)}`.trim();
}

function down(theme: Theme, device: Device): string {
  return `@media screen ${getMaxWidthQuery(theme, device)}`.trim();
}
...
function only(theme: Theme, device: Device): string {
  ...
  return `@media screen ${minWidth} ${maxWidth}`.trim();
}
