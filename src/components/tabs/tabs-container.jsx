...
export default class TabsContainer extends PureComponent {
  ...
  set currentTab(tabName) {
    this.setState({ selectedTab: tabName }, () => this.animateBar(this.state.selectedTab));
  }
  ...
  handleKeyPress = (ev) => {
    ...
    if (TabsContainer.switchOnKeyCodes.includes(keyCode)) {
      this.setState((state) => {
    ...
    if (TabsContainer.moveDirectionsOnKeyCodes[keyCode]) {
      this.setState(({ focusedTab }) => {
        const direction = TabsContainer.moveDirectionsOnKeyCodes[keyCode];
